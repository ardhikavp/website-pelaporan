    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('safety-behavior-checklist.update', $answer->id) }}">
                        @csrf
                        @method('PUT')

                        <div class="form-group">
                            <label for="date_finding">Date Finding</label>
                            <input type="date" name="date_finding" id="date_finding" class="form-control" value="{{ $answer->date_finding }}" required>
                        </div>

                        <div class="form-group">
                            <label for="operation_name">Operation Name</label>
                            <input type="text" name="operation_name" id="operation_name" class="form-control" value="{{ $answer->operation_name }}" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="company_id" id="company_id" class="form-control" required>
                                @foreach ($companies as $company)
                                    <option value="{{ $company->id }}" {{ $answer->company_id == $company->id ? 'selected' : '' }}>
                                        {{ $company->company }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="mb-3">
                    <label>
                        <input type="radio"
                            ...
                            value="safe"
                            {{ isset($answer->answer[$checklist->category][$key]['answer']) && $answer->answer[$checklist->category][$key]['answer'] === 'safe' ? 'checked' : '' }}>
                        Safe
                    </label>
                </div>
            </td>
                ...
                <div class="mb-3">
                    <label>
                        <input type="radio"
                            name="answer[{{ $checklist->category }}][{{ $key }}]"
                            value="unsafe"
                            {{ isset($answer->answer[$checklist->category][$key]['answer']) && $answer->answer[$checklist->category][$key]['answer'] === 'unsafe' ? 'checked' : '' }}>
                        Unsafe
                    </label>
                </div>
            </td>
                ...
                <div class="mb-3">
                    <label>
                        <input type="radio"
                            name="answer[{{ $checklist->category }}][{{ $key }}]"
                            value="n/a"
                            {{ isset($answer->answer[$checklist->category][$key]['answer']) && $answer->answer[$checklist->category][$key]['answer'] === 'n/a' ? 'checked' : '' }}>
                        N/A
                    </label>
                </div>
            </td>
